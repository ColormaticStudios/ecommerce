<script lang="ts">
	...
	const savePrompt = createAdminSavePrompt({
		navigationMessage: "You have unsaved storefront changes. Leave this section and discard them?",
	});
	...
	function buildPersistedStorefrontPayload(
		...
		payload.homepage_sections = payload.homepage_sections.map((section) => {
			...
			if (section.type === "promo_cards") {
				return {
					...section,
					promo_cards: (section.promo_cards ?? [])
						.slice(0, section.promo_card_limit ?? 1)
						.filter(
							(card) =>
								card.title.trim() !== "" ||
	...
	async function handleHeroImageUpload(section: StorefrontHomepageSectionModel, event: Event) {
		...
		if (!file.type.startsWith("image/")) {
			notices.setError("Hero background must be an image file.");
			return;
		}
		...
		try {
			...
			notices.setSuccess("Hero background uploaded. Save storefront to publish this image.");
		} catch (err) {
			console.error(err);
			notices.setError("Unable to upload hero background image.");
		} finally {
			uploadingHero = false;
		}
	}
	...
	async function saveStorefrontSettings() {
		...
		try {
			...
			if (missingFields.length > 0) {
				notices.setError(
					`Please fill the required fields before saving: ${missingFields.join("; ")}.`
				);
				return;
			}
			...
			clearAllHeroPreviews();
			hydrateDraft(response.settings);
			notices.setSuccess("Storefront draft saved.");
		} catch (err) {
			console.error(err);
			notices.setError(`Unable to save storefront settings: ${getReadableSaveError(err)}.`);
		} finally {
			saving = false;
		}
	}
	...
	async function publishStorefrontDraft() {
		...
		try {
			...
			clearAllHeroPreviews();
			hydrateDraft(response.settings);
			notices.setSuccess("Storefront draft published.");
		} catch (err) {
			console.error(err);
			notices.setError(`Unable to publish storefront draft: ${getReadableSaveError(err)}.`);
		} finally {
			publishing = false;
		}
	}
	...
	async function discardStorefrontDraft() {
		...
		try {
			...
			clearAllHeroPreviews();
			hydrateDraft(response.settings);
			notices.setSuccess("Storefront draft discarded.");
		} catch (err) {
			console.error(err);
			notices.setError(`Unable to discard storefront draft: ${getReadableSaveError(err)}.`);
		} finally {
	...
	async function previewStorefrontDraft() {
		...
		try {
			...
			notices.setSuccess("Opened storefront draft preview in a new tab.");
		} catch (err) {
			console.error(err);
			if (previewWindow && !previewWindow.closed) {
				previewWindow.close();
			}
			notices.setError(`Unable to open storefront draft preview: ${getReadableSaveError(err)}.`);
			void loadPreviewState();
		} finally {
	...
	$effect(() => {
		...
		savePrompt.blocked = saving || uploadingHero || publishing;
		savePrompt.saveAction = hasUnsavedChanges ? saveStorefrontSettings : null;
	});
</script>
			...
			<div class="mt-6 space-y-6">
				...
				<AdminSurface variant="subsurface">
						...
						{#each draft.homepage_sections as section, sectionIndex (section.id)}
							<div class="rounded-lg border border-stone-200 p-4 dark:border-stone-800">
								...
								{#if section.type === "products" && section.product_section}
									<div class="mt-4 grid gap-3">
										...
										<div class="grid gap-3 md:grid-cols-4">
											<Dropdown tone="admin" bind:value={section.product_section.source}>
											...
											<Dropdown tone="admin" bind:value={section.product_section.order}>
										...
										<div class="grid gap-3 md:grid-cols-4">
											...
											<Dropdown tone="admin" bind:value={section.product_section.image_aspect}>
								...
								{#if section.type === "hero" && section.hero}
									...
									<div class="mt-4 grid gap-3">
										<TextInput tone="admin" placeholder="Eyebrow" bind:value={hero.eyebrow} />
										<TextInput tone="admin" placeholder="Hero title" bind:value={hero.title} />
										<TextArea
											tone="admin"
											class="min-h-22"
										...
										<div class="grid gap-3 md:grid-cols-2">
											<div class="grid grid-cols-2 gap-2">
												...
												<TextInput
													...
													bind:value={hero.primary_cta.url}
												/>
											</div>
											<div class="grid grid-cols-2 gap-2">
												<TextInput
											...
											{#each (section.promo_cards ?? []).slice(0, section.promo_card_limit ?? 1) as card, cardIndex (cardIndex)}
												<div class="rounded-lg border border-gray-200 p-3 dark:border-gray-800">
													...
													<div class="grid gap-2 md:grid-cols-2">
														<TextInput tone="admin" placeholder="Kicker" bind:value={card.kicker} />
														<TextInput tone="admin" placeholder="Title" bind:value={card.title} />
														<TextArea
															tone="admin"
															class="min-h-20 md:col-span-2"
														...
														<TextInput
															...
															bind:value={card.image_url}
														/>
														<TextInput
															tone="admin"
															placeholder="Link label"
															bind:value={card.link.label}
														/>
														<TextInput
															tone="admin"
															class="md:col-span-2"
					...
					<div class="mt-4 flex flex-wrap items-center justify-end gap-2">
						<Dropdown tone="admin" full={false} bind:value={newSectionType}>
							<option value="hero">Hero</option>
							<option value="products">Products</option>
							<option value="promo_cards">Promo cards</option>
							<option value="badges">Badges</option>
						...
						<Button
							...
							onclick={() => {
								...
								draft.homepage_sections = [
									...draft.homepage_sections,
									createSection(newSectionType),
								];
								syncManualProductInputFromDraft();
							}}
						>
				...
				<section class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
					...
					<div class="mt-4 grid gap-3 md:grid-cols-3">
						<TextInput tone="admin" placeholder="Brand name" bind:value={draft.footer.brand_name} />
						<TextInput tone="admin" placeholder="Tagline" bind:value={draft.footer.tagline} />
						<TextInput
							tone="admin"
							placeholder="Copyright text"
							bind:value={draft.footer.copyright}
						/>
					</div>
					<TextInput
						tone="admin"
...
<AdminFloatingNotices
	...
	unsavedMessage="You have unsaved storefront changes."
