<script lang="ts">
	...
	import {
		...
		type CmsCampaignTemplateKey as CampaignTemplateKey,
		type CmsGlobalRegionType as GlobalRegionType,
	} from "$lib/admin/cms/blockFactory";
	import CmsDeliveryPanel from "$lib/admin/cms/CmsDeliveryPanel.svelte";
	import CmsGlobalsPanel from "$lib/admin/cms/CmsGlobalsPanel.svelte";
	import CmsLocalizationPanel from "$lib/admin/cms/CmsLocalizationPanel.svelte";
	import CmsNavigationPanel from "$lib/admin/cms/CmsNavigationPanel.svelte";
	import CmsOperationsPanel from "$lib/admin/cms/CmsOperationsPanel.svelte";
	import CmsPagesPanel from "$lib/admin/cms/CmsPagesPanel.svelte";
	import CmsSeoPanel from "$lib/admin/cms/CmsSeoPanel.svelte";
	import {
		navigationItemExists,
	...
	import {
		assessCmsBlocks,
		hasBlockingCmsBlocks,
		serializeCmsBlock,
		type CmsEditableBlock,
	} from "$lib/admin/cms/blocks";
	...
	const hasKnownInvalidBlocks = $derived(
		activeTab === "pages"
			? hasBlockingCmsBlocks(pageBlocks)
			: activeTab === "global"
				? hasBlockingCmsBlocks(globalBlocks)
				: false
	);
	const canPublish = $derived(
		!publishing &&
			!hasUnsavedChanges &&
			!hasKnownInvalidBlocks &&
			hasDraftChanges &&
			(activeTab === "navigation"
	...
	function defaultPageBlocks(): EditableBlock[] {
		return createDefaultPageBlocks(editorId);
	}

	function defaultGlobalBlocks(region: GlobalRegionType): EditableBlock[] {
		return createDefaultGlobalBlocks(region, editorId);
	}

	function campaignTemplateBlocks(template: CampaignTemplateKey): EditableBlock[] {
		return createCampaignTemplateBlocks(template, editorId);
	}
	...
	function blocksFromPayload(
		...
		return assessCmsBlocks(payload?.blocks, editorId, fallback);
	}

	function payloadFromBlocks(blocks: EditableBlock[]): CmsPagePayload {
		const serialized = blocks.map(serializeCmsBlock);
		// Admin drafts intentionally preserve malformed JSON for repair. Publication
		// validation rejects it before this payload can become public content.
		return { blocks: serialized as CmsPagePayload["blocks"] };
	}
	...
	function openGlobal(region: CmsGlobalRegionResponse) {
		...
		globalBlocks = blocksFromPayload(
			globalPayloadFromResponse(region),
			defaultGlobalBlocks(globalRegion)
		);
		markSaved();
	}
	...
	function createBlock(type: CmsContentBlock["type"]): EditableBlock {
		return createCmsBlock(type, editorId);
	}
	...
	function navigationSelectionExists(item: SelectedNavigationItem): boolean {
		return navigationItemExists(item, navigationDropdowns, navigationCustomItems, navigationRows);
	}
...
<div class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			...
			{#snippet master()}
				...
				>
					{#if activeTab === "pages"}
						<CmsPagesPanel
							{pages}
							selectedId={selected.kind === "page" ? selected.id : null}
							onSelect={selectPage}
						/>
					{:else if activeTab === "navigation"}
						<CmsNavigationPanel
							...
							customItems={navigationCustomItems}
							rows={navigationRows}
							selected={selectedNavigationItem}
							onSelect={(item) => (selectedNavigationItem = item)}
						/>
					{:else}
						<CmsGlobalsPanel
							regions={globalRegions}
							selectedId={selected.kind === "global" ? selected.id : null}
	...
	{:else}
		<CmsOperationsPanel
			...
			{operations}
			{operationsLoading}
			onAddRole={addGovernanceRole}
			onUpdateRole={updateGovernanceRole}
			onRemoveRole={removeGovernanceRole}
			onSaveGovernance={() => void saveGovernance()}
			onRefreshOperations={() => void refreshOperations()}
			onRetryInvalidation={(id) => void retryInvalidation(id)}
		/>
	{/if}
	...
	{#if activeTab === "pages"}
		<CmsLocalizationPanel
			...
			{pageVariants}
			{selectedVariantId}
			currentVariant={selectedPageVariant}
			bind:variantLocale
			bind:variantMarket
			bind:variantPath
			bind:variantSlug
			...
			{variantSaving}
			bind:variantComment
			bind:selectedPageSectionType
			{pageSectionOptions}
			{blockEditor}
			{addLocale}
			saveLocales={() => void saveLocales()}
			{newPageVariant}
			{selectPageVariant}
			{addBlock}
			savePageVariant={() => void savePageVariant()}
			{transitionPageVariant}
			removePageVariant={() => void removePageVariant()}
		/>
		...
		<CmsSeoPanel
			...
			bind:seoTitle
			bind:seoDescription
			bind:seoCanonicalURL
			bind:seoRobots
			bind:seoOGTitle
			bind:seoOGDescription
			bind:seoOGImageMediaID
			bind:seoTwitterCard
			bind:seoTwitterTitle
			bind:seoTwitterDescription
			bind:seoTwitterImageMediaID
			bind:seoJSONLDType
			bind:seoJSONLDName
			{seoIssues}
			{uploadSEOMedia}
			savePageSEO={() => void savePageSEO()}
		/>
		<CmsDeliveryPanel
			selectedPageId={selected.id}
			{deliveryLoading}
			{deliverySaving}
			{deliverySaveDisabled}
			bind:scheduleEnabled
			bind:schedulePublishAt
			bind:scheduleExpiryEnabled
			bind:scheduleUnpublishAt
			bind:scheduleTimezone
			{scheduleStatus}
			{scheduleLastTransitionAt}
			{deliveryPublications}
			bind:deliveryRules
			bind:experimentEnabled
			bind:experimentName
			bind:experimentStatus
			bind:experimentStickyKey
			bind:experimentStartsAt
			bind:experimentEndsAt
			bind:controlVersionId
			bind:variantVersionId
			bind:controlAllocation
			{pageVersionOptions}
			publishedVersionId={selectedPage?.published_version?.id ?? null}
			{addDeliveryRule}
			{toggleRuleDevice}
			savePageDelivery={() => void savePageDelivery()}
		/>
